...
while True:
    ...
    cam_yellow = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    #definimos el rango de colores que queremos detectar, en este caso el amarillo
    amarillo_bajo = np.array([20, 100, 100], dtype=np.uint8)
    amarillo_alto = np.array([30, 255, 255], dtype=np.uint8)

    #definimos la mascara detectar los colores
    mask = cv2.inRange(cam_yellow, amarillo_bajo, amarillo_alto) #para crear una mascara que detecte los colores amarillos, se utiliza la funcion inRange de OpenCV, que toma como parametros la imagen en el espacio de color HSV, el valor minimo del color a detectar (amarillo_bajo) y el valor maximo del color a detectar (amarillo_alto). la funcion inRange devuelve una mascara binaria, donde los pixeles que cumplen con el rango de colores especificado se establecen en 255 (blanco) y los pixeles que no cumplen con el rango se estable
    ...
    #aqui es donde el amarillo se vera blanco y lo demas negro. 
